...
The refreshed GOV.UK brand is available and turned on by default in the `bng-metric-digital-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag bng-metric-digital-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 bng-metric-digital-prototype:development
```
...
```bash
docker build --no-cache --tag bng-metric-digital-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 bng-metric-digital-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash bng-metric-digital-prototype
```
