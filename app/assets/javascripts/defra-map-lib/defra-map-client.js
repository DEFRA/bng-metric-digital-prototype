...
;(function (window) {
  ...
  const DefraEventEmitter =
    window.DefraMapLib && window.DefraMapLib.DefraEventEmitter
  ...
  function DefraMapClient(options) {
    ...
    if (!this._targetEl) {
      throw new Error(
        'DefraMapClient requires a target element (id or HTMLElement).'
      )
    }
    ...
    this._minZoomForSnap =
      typeof this._osFeatures.minZoomForSnap === 'number'
        ? this._osFeatures.minZoomForSnap
        : 14
    this._fetchThrottleMs =
      typeof this._osFeatures.fetchThrottleMs === 'number'
        ? this._osFeatures.fetchThrottleMs
        : 300
    this._snapTolerancePx =
      typeof this._osFeatures.snapTolerancePx === 'number'
        ? this._osFeatures.snapTolerancePx
        : 25
    this._closeTolerancePx =
      typeof this._osFeatures.closeTolerancePx === 'number'
        ? this._osFeatures.closeTolerancePx
        : 10
    this._simplifyTolerance =
      typeof this._osFeatures.simplifyTolerance === 'number'
        ? this._osFeatures.simplifyTolerance
        : 0
    this._maxFeaturesPerRequest =
      typeof this._osFeatures.maxFeaturesPerRequest === 'number'
        ? this._osFeatures.maxFeaturesPerRequest
        : 100
  ...
  DefraMapClient.prototype.init = async function () {
    ...
    const minZoom =
      typeof this._options.minZoom === 'number' ? this._options.minZoom : 0
    const maxZoom =
      typeof this._options.maxZoom === 'number' ? this._options.maxZoom : 16
    ...
    if (
      ...
      throw new Error(
        'DefraMapClient tiles config missing. Provide tiles.collectionId/crs/tileMatrixSetUrl/styleUrl/tilesUrlTemplate.'
      )
    }
  ...
  Object.defineProperty(DefraMapClient.prototype, 'parcelsTotalAreaSqm', {
    get: function () {
      ...
      return this._habitatParcels.reduce(
        (sum, p) => sum + p.feature.getGeometry().getArea(),
        0
      )
    }
  })
  ...
  DefraMapClient.prototype.setBoundaryFromCoordinates = function (coords) {
    ...
    this._emitter.emit('boundary:changed', {
      areaSqm: this.boundaryAreaSqm,
      source: 'setBoundaryFromCoordinates'
    })
    return true
  }
  ...
  DefraMapClient.prototype.startDrawing = function () {
    ...
    if (this._mode === 'red-line-boundary' && this._polygonComplete) {
      this._emitter.emit('validation:error', {
        message: 'A boundary already exists. Clear it before drawing a new one.'
      })
      return
    }
  ...
  DefraMapClient.prototype.finishDrawing = function () {
    ...
    if (this._currentPolygonCoords.length < 3) {
      this._emitter.emit('validation:error', {
        message: 'Need at least 3 points to complete a polygon.'
      })
      return
    }
  ...
  DefraMapClient.prototype.saveBoundary = async function () {
    ...
    const payload = this.exportBoundaryGeoJSON({
  ...
  DefraMapClient.prototype.saveParcels = async function () {
    ...
    const payload = this.exportParcelsGeoJSON({
      dataProjection: this._projection
    })
  ...
  DefraMapClient.prototype._emitViewChanged = function () {
    ...
    this._emitter.emit('view:changed', {
      ...
      snappingAvailable:
        typeof zoom === 'number' ? zoom >= this._minZoomForSnap : false
    })
  }
  ...
  DefraMapClient.prototype._styleDrawFeature = function (feature) {
    ...
    if (type === 'vertex') {
      ...
      if (isBeingDragged) {
        ...
        isHovered &&
        (this._polygonComplete || this._editingParcelIndex >= 0)
      ) {
        radius = 5
    ...
    if (type === 'polygon' || type === 'parcel') {
      ...
      if (this._mode === 'habitat-parcels') {
        const colors =
          this._parcelColors[colorIndex % this._parcelColors.length]
  ...
  DefraMapClient.prototype._handlePointerMove = function (evt) {
    ...
    const canEdit =
      (this._polygonComplete && !this._isDrawing) ||
      this._editingParcelIndex >= 0
    if (canEdit) {
  ...
  DefraMapClient.prototype._handlePointerDown = function (evt) {
    ...
    const canEdit =
      (this._polygonFeature && this._polygonComplete) ||
      this._editingParcelIndex >= 0
    if (!canEdit) return

    if (
      this._ghostVertex &&
      this._ghostVertexCoord &&
      this._ghostVertexInsertIndex >= 0
    ) {
      this._insertNewVertex(
        this._ghostVertexCoord,
        this._ghostVertexInsertIndex
      )
      this._clearGhostVertex()
      evt.stopPropagation()
      evt.preventDefault()
  ...
  DefraMapClient.prototype._placeVertex = function (coordinate, isFirst) {
    ...
    const colorIndex =
      this._mode === 'habitat-parcels' ? this._habitatParcels.length : 0
  ...
  DefraMapClient.prototype._updateHoverMarker = function (
    coordinate,
    snapType
  ) {
    this._hoverSource.clear()
  ...
  DefraMapClient.prototype._updateLivePolygon = function (snapCoord) {
    ...
    if (tempCoords.length >= 3) {
      try {
        const tempPoly = new ol.geom.Polygon([
          [...this._currentPolygonCoords, this._currentPolygonCoords[0]]
        ])
  ...
  DefraMapClient.prototype._closePolygon = function () {
    ...
    if (this._mode === 'habitat-parcels') {
      ...
      this._emitter.emit('parcel:added', {
        index: addedIndex,
        id: id,
        areaSqm: parcel.feature.getGeometry().getArea()
      })
      this._emitter.emit('parcels:changed', {
        count: this._habitatParcels.length,
        totalAreaSqm: this.parcelsTotalAreaSqm
  ...
  DefraMapClient.prototype._checkPolygonEdgeHover = function (
    pixel,
    snapCoord
  ) {
    const canEdit =
  ...
  DefraMapClient.prototype._showGhostVertex = function (
    coordinate,
    insertIndex
  ) {
    this._clearGhostVertex()
  ...
  DefraMapClient.prototype._insertNewVertex = function (
    coordinate,
    insertIndex
  ) {
    this._currentPolygonCoords.splice(insertIndex, 0, [...coordinate])
    const colorIndex = this._polygonFeature
      ? this._polygonFeature.get('colorIndex')
      : 0
    ...
    this._currentPolygonCoords[this._currentPolygonCoords.length - 1] = [
      ...this._currentPolygonCoords[0]
    ]
    if (this._polygonFeature) {
      this._polygonFeature
        .getGeometry()
        .setCoordinates([this._currentPolygonCoords])
    }
    ...
    if (this._editingParcelIndex >= 0) {
      ...
      this._emitter.emit('parcel:changed', {
        index: this._editingParcelIndex,
        areaSqm: parcel.feature.getGeometry().getArea()
      })
      this._emitter.emit('parcels:changed', {
        count: this._habitatParcels.length,
  ...
  DefraMapClient.prototype._updateDraggedVertex = function (snapCoord) {
    ...
    if (this._draggedVertexIndex === 0) {
      ...
      this._draggedVertexIndex ===
      this._currentPolygonCoords.length - 1
    ) {
  ...
  DefraMapClient.prototype._handleRemoveClick = function (evt) {
    ...
    if (this._mode === 'red-line-boundary') {
      ...
      if (
        feature &&
        (feature.get('type') === 'polygon' || feature === this._polygonFeature)
      ) {
        this.clearBoundary()
        this._removeActive = false
  ...
  DefraMapClient.prototype._handleRemoveHover = function (evt) {
    ...
    // Update cursor
    this._map.getTargetElement().style.cursor = hoveredFeature
      ? 'pointer'
      : 'crosshair'
  }
  ...
  DefraMapClient.prototype.getDebugInfo = function () {
    return {
      ...
      osFeatures: {
        ...
        featureCount: this._snapIndexSource
          ? this._snapIndexSource.getFeatures().length
          : 0
      },
      drawing: {
        isDrawing: this._isDrawing,
        polygonComplete: this._polygonComplete
      },
      parcels: {
        count: this._habitatParcels.length,
        editingIndex: this._editingParcelIndex,
        selectedIndex: this._selectedParcelIndex
      },
      fill: {
        active: this._fillActive,
        mode: this._fillMode,
