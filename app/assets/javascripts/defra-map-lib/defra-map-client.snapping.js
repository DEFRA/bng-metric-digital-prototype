...
;(function (window) {
  ...
  if (!DefraMapClient) {
    throw new Error(
      'defra-map-client.snapping.js requires window.DefraMapClient to be loaded first.'
    )
  }
  ...
  DefraMapClient.prototype.setSnappingEnabled = function (enabled) {
    ...
    this._emitter.emit('snapping:osFeaturesChanged', {
      enabled: this._snappingEnabled
    })
  }

  DefraMapClient.prototype.setSnapToBoundaryVertices = function (enabled) {
    this._snapToBoundaryVertices = !!enabled
    this._emitter.emit('snapping:boundaryVerticesChanged', {
      enabled: this._snapToBoundaryVertices
    })
  }

  DefraMapClient.prototype.setSnapToBoundaryEdges = function (enabled) {
    this._snapToBoundaryEdges = !!enabled
    this._emitter.emit('snapping:boundaryEdgesChanged', {
      enabled: this._snapToBoundaryEdges
    })
  }

  DefraMapClient.prototype.setSnapToParcelVertices = function (enabled) {
    this._snapToParcelVertices = !!enabled
    this._emitter.emit('snapping:parcelVerticesChanged', {
      enabled: this._snapToParcelVertices
    })
  }

  DefraMapClient.prototype.setSnapToParcelEdges = function (enabled) {
    this._snapToParcelEdges = !!enabled
    this._emitter.emit('snapping:parcelEdgesChanged', {
      enabled: this._snapToParcelEdges
    })
  }
  ...
  DefraMapClient.prototype._fetchSnapData = async function () {
    ...
    if (
      this._lastFetchExtent &&
      ol.extent.equals(extent, this._lastFetchExtent)
    ) {
      return
    }
  ...
  DefraMapClient.prototype._fetchLayerData = async function (
    baseUrl,
    collectionId,
    extent
  ) {
    ...
    if (this._snappingEnabled) {
      ...
      features.forEach((feature) => {
        ...
        if (type === 'LineString') checkClosest(geom)
          ...
          geom.getLineStrings().forEach((ls) => checkClosest(ls))
        else if (type === 'Polygon') checkClosest(geom.getLinearRing(0))
        else if (type === 'MultiPolygon')
          geom
            .getPolygons()
            .forEach((poly) => checkClosest(poly.getLinearRing(0)))
      })
    }

    const result = {
      coordinate: snapPoint || coordinate,
      snapType: snapPoint ? snapType : this._snapType.NONE
    }
    return result
  }
  ...
  DefraMapClient.prototype._clampToBoundary = function (coordinate) {
    ...
    if (this._boundaryPolygon.intersectsCoordinate(coordinate))
      return coordinate
    const ring = this._boundaryPolygon.getLinearRing(0)
