...
(function(window) {
  ...
  if (!DefraMapClient) {
    throw new Error('defra-map-client.keyboard.js requires window.DefraMapClient to be loaded first.');
  }
  ...
  const KEYBOARD_SHORTCUTS = [
    { category: 'Navigation', shortcuts: [
      ...
      { keys: 'Shift + Arrow keys', description: 'Pan the map (fine control)' },
    ...
    { category: 'Drawing & Selection', shortcuts: [
      { keys: 'Ctrl + Space', description: 'Place point, select fill area, or set slice point' },
      { keys: 'Enter', description: 'Finish drawing and accept shape' },
      { keys: 'Escape', description: 'Cancel current operation or close menu' }
    ]}
  ];
  ...
  DefraMapClient.prototype._setupKeyboardInteraction = function() {
    ...
    target.setAttribute('aria-label', 'Interactive map. Use arrow keys to pan, plus and minus to zoom. Press Ctrl+K for keyboard shortcuts.');
  ...
  DefraMapClient.prototype._updateSlicePreviewForKeyboard = function(snapCoord) {
    ...
    if (sliceSnap) {
      ...
      if (sliceSnap.isVertex) {
        featureType = sliceSnap.sourceType === 'parcel' ? 'parcel-vertex-hover' : 'boundary-vertex-hover';
      } else {
  ...
  DefraMapClient.prototype._handleGlobalKeydown = function(e) {
    ...
    if (e.key === 'Escape') {
      ...
      if (drawer && drawer.classList.contains('defra-map-controls__drawer--open')) {
        e.preventDefault();
        e.stopPropagation();
  ...
  DefraMapClient.prototype._toggleDrawerMenu = function() {
    ...
    const hamburgerBtn = document.querySelector('.defra-map-controls__hamburger');
  ...
  DefraMapClient.prototype._handleKeyboardFillSelect = async function() {
    ...
    const featureCount = this._snapIndexSource ? this._snapIndexSource.getFeatures().length : 0;
    console.log('[KB Fill] zoom:', zoom, 'features:', featureCount, 'minZoom:', this._minZoomForSnap);
    ...
    if (featureCount === 0) {
      ...
      console.log('[KB Fill] After fetch:', this._snapIndexSource ? this._snapIndexSource.getFeatures().length : 0);
    }

    const mapSize = this._map.getSize();
    const centerPixel = [mapSize[0] / 2, mapSize[1] / 2];
    const centerCoord = this._map.getCoordinateFromPixel(centerPixel);
    console.log('[KB Fill] centerPixel:', centerPixel, 'centerCoord:', centerCoord);
    ...
    this._announceAction(`Polygon ${this._fillSelected.length > 0 ? 'selected' : 'deselected'}`);
  };
  ...
  DefraMapClient.prototype._handleKeyboardSliceSelect = function() {
    ...
    if (!this._sliceStart) {
      ...
      if (!snapInfo) {
        this._announceAction('Please position crosshair on a boundary or parcel edge');
        return;
      }
      ...
      this._announceAction('Slice start point set. Move to end point and press Ctrl+Space again');
    ...
    if (!snapInfo) {
      this._announceAction('Please position crosshair on the same polygon to complete slice');
      return;
    }
  ...
  const originalSetupMapEventHandlers = DefraMapClient.prototype._setupMapEventHandlers;
