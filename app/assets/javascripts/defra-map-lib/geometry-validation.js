...
(function(window) {
  ...
  GeometryValidation.correctGeometryToBoundary = function(parcelGeom, boundaryPolygon) {
    if (!boundaryPolygon) {
      return parcelGeom;
    }
    ...
    for (let i = 0; i < parcelCoords.length; i++) {
      ...
      if (!snapped) {
        for (let j = 0; j < boundaryCoords.length - 1; j++) {
          ...
          const closestPoint = GeometryValidation.getClosestPointOnSegment(coord, edgeStart, edgeEnd);
  ...
  GeometryValidation.getClosestPointOnSegment = function(point, segStart, segEnd) {
    const x = point[0];
    const y = point[1];
    const x1 = segStart[0];
    const y1 = segStart[1];
  ...
  GeometryValidation.arePolygonsAdjacent = function(polygon1, polygon2) {
    ...
    for (let i = 0; i < coords1.length - 1; i++) {
      ...
      for (let j = 0; j < coords2.length - 1; j++) {
        ...
        if (GeometryValidation.doSegmentsOverlap(seg1Start, seg1End, seg2Start, seg2End)) {
          return true;
        }
      }
    }
