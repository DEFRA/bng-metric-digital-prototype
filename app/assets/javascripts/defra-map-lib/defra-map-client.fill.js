...
(function(window) {
  ...
  if (!DefraMapClient) {
    throw new Error('defra-map-client.fill.js requires window.DefraMapClient to be loaded first.');
  }
  if (!GeometryValidation) {
    throw new Error('defra-map-client.fill.js requires window.DefraMapLib.GeometryValidation to be loaded first.');
  }
  ...
  DefraMapClient.prototype._updateFillPreviewLayer = function() {
    ...
    console.log('[FillPreview] Update called, ID:', callId, 'selections:', this._fillSelected.length);
    ...
    console.log('[FillPreview] Features in preview:', this._fillPreviewSource.getFeatures().length, 'callId:', callId);
