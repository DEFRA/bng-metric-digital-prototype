...
window.GOVUKPrototypeKit.documentReady(() => {
  ...
  async function saveParcels() {
    ...
    if (remaining < -AREA_TOLERANCE_SQM) {
      showStatus('Parcel areas exceed boundary area. Please adjust parcels.', 'error');
      return;
    }
  ...
  function renderAreaDisplay() {
    ...
    if (boundaryEl && client.boundaryAreaSqm) {
      boundaryEl.textContent = (client.boundaryAreaSqm / 10000).toFixed(2) + ' ha';
    }

    if (totalEl) {
      totalEl.textContent = (client.parcelsTotalAreaSqm / 10000).toFixed(2) + ' ha';
    }
    ...
    if (remainingEl) {
      ...
      if (Math.abs(remainingHa) < 0.005) {
        ...
        remainingEl.className = 'map-area-display__data map-area-display__data--success';
      } else if (remainingHa < 0) {
        remainingEl.textContent = remainingHa.toFixed(2) + ' ha';
        remainingEl.className = 'map-area-display__data map-area-display__data--warning';
      } else {
        remainingEl.textContent = remainingHa.toFixed(2) + ' ha';
